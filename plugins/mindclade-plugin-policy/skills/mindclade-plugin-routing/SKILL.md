---
name: mindclade-plugin-routing
description: >-
  Mindclade's plugin routing policy. Use at the start of any non-trivial task in
  the Mindclade monorepo, estate, or SDK Foundry to decide which plugin or skill
  handles it, and to avoid bundled skills that contradict Production Plan v3.1.
  Trigger when choosing a plugin, agent, or skill; when asked which tool to use;
  before spawning agents or teams; and before any release, migration, or push to
  a remote. Covers protobuf contracts, OpenAPI projection, PostgreSQL and Pub/Sub,
  GCP and Cloud Run, GitHub Actions, the training vertical, and release guardrails.
---

# Mindclade plugin routing policy

Authority for every routing decision here is *Mindclade Authoritative Protocol, SDK Foundry, and
Product Monorepo Production Plan v3.1*. Where this policy and a plugin's own advice disagree, this
policy wins — the excluded skills in §3 contradict the plan directly.
...
| `api-batch-processor` / `/implement-batch-processing` | **Highest misroute risk in the whole catalog** — the name reads like §11.5 (jobs, batching, progress) but every prescription conflicts. It specifies Redis-backed progress tracking; §11.5 requires `operation_events` written in the same transaction as the mutation, with a gap-free `sequence` from `operations.next_sequence` under row lock, because *"watch consistency must equal database consistency"*. It specifies Bull/BullMQ, Celery or SQS; §11.6 specifies a transactional outbox onto Pub/Sub. It scaffolds Node.js `src/routes/batch.js` and `.js` workers; the runtime is Go. It offers heartbeat-timeout failure detection; §11.5 requires monotonic fencing tokens so a stale attempt fails *even while it retains network connectivity*. Adopting it would build the exact second-consistency-domain the plan forbids. |
